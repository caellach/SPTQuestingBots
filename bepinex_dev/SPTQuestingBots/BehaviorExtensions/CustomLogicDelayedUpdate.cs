...
namespace SPTQuestingBots.BehaviorExtensions
{
    public abstract class CustomLogicDelayedUpdate : CustomLogic
    {
        ...
        {
            var curPath = BotOwner.Mover.GetCurPath();
            if (curPath == null)
            {
                return false;
            }
            ...
            if (currentCornerIndex == curPath.Length - 1)
            {
                return true;
            }

            ObjectiveManager.LastCorner = curPath[currentCornerIndex];

            Vector3 currentSegment = curPath[currentCornerIndex] - curPath[currentCornerIndex - 1];
            Vector3 nextSegment = curPath[currentCornerIndex + 1] - curPath[currentCornerIndex];
